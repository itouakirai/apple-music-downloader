...
func (r *Runner) getUrlArtistName(artistUrl string, token string) (string, string, error) {
	...
	query := url.Values{}
	query.Set("l", r.Config.General.Language)
...
func (r *Runner) checkArtist(artistUrl string, token string, relationship string) ([]string, error) {
	...
	var options [][]string
	for {
		req, err := http.NewRequest("GET", fmt.Sprintf("https://amp-api.music.apple.com/v1/catalog/%s/artists/%s/%s?limit=100&offset=%d&l=%s", storefront, artistId, relationship, Num, r.Config.General.Language), nil)
		if err != nil {
			return nil, err
		}
		req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", token))
